const Signature = class {
    ...
    initialize = (signatureCanvasWrapper, signatureSelect, signatureType, signatureField, signaturePreview) => {
        this.signatureCanvasWrapper = signatureCanvasWrapper;
        this.signatureSelect = signatureSelect;
    ...
    bindInputChange = () => {
        $(this.signatureField).change(() => {
            console.log("Change triggered!");
            const file = $(this.signatureField).prop("files")[0];
            if (!file) return;
    ...
    bindSignatureTypeToggle = () => {
        console.log("Calling bindSignatureTypeToggle with", this.signatureType);
        $(`input[name=${this.signatureType}]`).change(this.handleSignatureTypeChange);
    }

    handleSignatureTypeChange = () => {
        this.type = $(`input[name=${this.signatureType}]:checked`).val();
        if (this.type === "DRAW") {
            $(this.signatureCanvasWrapper).show();
            $(this.signatureSelect).hide();
        } else if (this.type === "SELECT") {
            $(this.signatureSelect).show();
            $(this.signatureCanvasWrapper).hide();
        }
    }
    ...
    getSignatureImage = () => {
        return this.type === "DRAW"
            ...
            : $(this.signaturePreview).attr("src");
    }
